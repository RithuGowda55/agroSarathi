...
agro_api_key = "d30ecc86880bf84c2609819752aa9706"
...
def delete_all_polygons(api_key):  
    ...
    try:  
        ...
        for polygon in polygons:  
            ...
            delete_url = f"http://api.agromonitoring.com/agro/1.0/polygons/{polygon_id}?appid={api_key}"  
            delete_response = requests.delete(delete_url)  
            delete_response.raise_for_status()  
            logging.info(f"Deleted polygon with ID: {polygon_id}")  

    except Exception as e:  
        logging.error(f"Error while deleting polygons: {e}")    
...
def create_polygon(api_key, coordinates, name="Temporary Polygon"):  
    ...
    if payload_size > 10 * 1024:  # Example size limit  
        logging.error("Payload size is too large. Aborting polygon creation.")  
        return None  
    ...
    try:  
        ...
        if "PayloadTooLargeError" in response.text:  
            logging.error("Polygon creation failed due to plan limits. Consider upgrading your plan.")  
...
def manage_polygon_and_fetch_soil(api_key, coordinates):
    ...
    if polyid:
        ...
        delete_all_polygons(agro_api_key)   
        return soil_data
    return None
